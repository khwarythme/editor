...
impl Display {
    ...
    fn update_line(&mut self, content: Arc<RwLock<VecDeque<VecDeque<char>>>>, row: usize) {
        let tmp_cursor_pos = row - self.point_in_file.row;
        let _ = queue!(self.out, MoveTo(0, tmp_cursor_pos as u16));

        let tmp_content = content.read().unwrap();
        let printstring = tmp_content.get(row).unwrap();
        ...
        let _ = queue!(
            self.out,
            MoveTo(COLUMN_LEFT_LIMIT, self.point.row as u16),
            Print(printstring)
        );
        let _ = queue!(
            self.out,
    ...
    pub async fn update_all(
        &mut self,
        content: Arc<RwLock<VecDeque<VecDeque<char>>>>,
    ) -> Result<(), String> {
        let mut row_index = 0;
        let tmp_content = content.read().unwrap();
    ...
    pub async fn move_cursor_nextpos(
        &mut self,
        direction: MoveDirection,
        content: Arc<RwLock<VecDeque<VecDeque<char>>>>,
    ) {
        let _ = queue!(self.out, cursor::Hide);
        let buf = content.read().unwrap();
        match direction {
            MoveDirection::Down => {
                if buf.len() <= self.point.row + self.point_in_file.row + 1 {
                    ...
                    self.update_line(Arc::clone(&content), self.get_cursor_coordinate_in_file().row);
                }
                self.pos_tmp.row = self.point_in_file.row + self.point.row;
                let col_length = buf[self.point.row + self.point_in_file.row].len();
            ...
            MoveDirection::Right => {
                if buf[self.point.row + self.point_in_file.row].len()
            ...
            MoveDirection::Tail => {
                self.point.column = buf[self.get_cursor_coordinate_in_file().row]
                    .len()
                    - 1;
            }
            MoveDirection::NewLine => {
                self.point.row += 1;
                self.point.column = 0;
                if self.point.row > self.wsize.row - 2 {
                    queue!(self.out, ScrollUp(1)).unwrap();
                    self.point_in_file.row += 1;
                    self.update_line(Arc::clone(&content), self.get_cursor_coordinate_in_file().row);
                }
        ...
        let column_pos = self.calc_char_disp_length(
            buf.get(self.point_in_file.row + self.point.row).unwrap().clone(),
            self.point.column as u16,
        );
        let _ = queue!(
            self.out,
...
pub async fn thread_main(
    ...
    let mut display = Display::new(Point {
        column: wcol as usize,
        row: wrow as usize,
    });
    display.init_window();
    display.update_all(Arc::clone(&contents)).await;
    loop {
        let msg = rx.recv().unwrap_or(Operation {
            sender: ProcessID::Show,
            code: OperationCode::DoNothing,
        });
        let result = match msg.code {
            OperationCode::Move(d) => {
                display.move_cursor_nextpos(d, Arc::clone(&contents));
                OperationCode::UpdatePosition(display.point_in_file)
            }
            OperationCode::Quit => break,
            _ => OperationCode::DoNothing,
        };
        if result != OperationCode::DoNothing {
            let sender = tx.read().unwrap();
